def menu():
    ...
    #for i in motchoisi:
    #    motenliste.append(i)
    #del motenliste[len(motenliste)-1]
    #print(motenliste)
...
def verif(lettrechoisi,motchoisi,lettredejaprop,pendu):
    ...
    if lettrechoisi in motchoisi.lower(): #verifie si la lettre fait partie du mot choisi
        ...
        for i in range (len(motchoisi)-1):
            motatrouver.append ("-")  #met des tirés à la place de chaque lettre du mot choisi
            positionLettre = motchoisi.lower().find(lettrechoisi)
            print("la position",positionLettre)
            lettre2=list(lettrechoisi)
            motatrouver= motatrouver[0:positionLettre] +  lettre2 + motatrouver[positionLettre+1:len(motchoisi)-1]

        #y=motchoisi.lower().find(lettrechoisi)
        #print("la lettre est en position",y)

        #motatrouver[y]=lettrechoisi
        #print(motatrouver)
        traits = "".join(motatrouver)#transformer list en str
        print(traits)
